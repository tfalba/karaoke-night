  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex justify-end">
        <NeonButton className="bg-[#13dcf6]/60 lowercase" onClick={onToggle}>
          {isOpen ? "- Now Singing" : "+ Now Singing"}
        </NeonButton>
      </div>
      <div
        className={[
          "overflow-hidden rounded-3xl border border-white/10 bg-[#13dcf6]/50 p-4",
      ...
      >
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/30 p-3">
          ...
          <div className="truncate text-xs text-white/80">
            {entry?.youtube?.channelTitle
              ? `Source: ${entry.youtube.channelTitle}`
              : ""}
          </div>
        </div>
      </div>
